...
func think():
	...
	animatedsprite.play()
	if fmod(attack, 500) == 0:
		dashing = dashing * -1
		attack = 0
		what = true
	attack += 1
	if what == true and dashing > 0:
		SPEED = 0
...
func _on_player_detector_body_entered(body: Node2D) -> void:
	if body.name == "player" :
		if body.is_dashing or invin == true:
			pass
		elif !body.is_jumping:
			die()
		else:
			pass
...
func die():
	...
	# Respawn logic
	global_position = Vector2(randi() % 51 - 2340, -53)  # Move enemy to respawn point
