...
@onready var explosion_timer: Timer = $ExplosionHitbox/ExplosionTimer


var player: CharacterBody2D = null

func _ready():
	set_explosion_hitbox(false)
...
func show_explosion_effect():
	set_explosion_hitbox(true)
	explosion_timer.start(0.2)  # Show hitbox for 0.2 seconds
	# Add any visual effects here (particles, animation, etc.)

func set_explosion_hitbox(active: bool):
	explosion_hitbox.set_deferred("monitoring", active)
	explosion_hitbox.set_deferred("monitorable", active)
	
	var collision_shape = explosion_hitbox.get_node("CollisionShapeBRuh")
